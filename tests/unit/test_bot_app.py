...
def mock_telegram_application(mocker):
    ...
    # Use a plain AsyncMock so we can freely attach attributes required for
    # the tests (spec_set=True prevents adding new attrs at runtime).
    mock_app_instance = AsyncMock()

    # Mock the builder chain: Application.builder().token().build()
    # We need to mock the `builder` method of the `Application` class itself.
    # The `mocker.patch` will replace `telegram.ext.Application.builder`.
    mock_builder = mocker.patch('telegram.ext.Application.builder')
    
    # Configure the return values of the chain
    # mock_builder.return_value is the result of Application.builder()
    # .token.return_value is the result of .token()
    # .build.return_value is the result of .build()
    ...
    # PTB v20 convenience method that wraps initialise/start/poll/idle/shutdown
    mock_app_instance.run_polling = AsyncMock()

    # Mock add_handler to prevent RuntimeWarning: coroutine was never awaited
    # add_handler is synchronous, so it should not be an AsyncMock
...
async def test_bot_app_initialization(mock_settings, mock_auth_manager, mock_telegram_application):
    ...
    # The mock_telegram_application fixture already sets up the mocker.patch for Application.builder
    # So we just need to create the BotApp instance.
